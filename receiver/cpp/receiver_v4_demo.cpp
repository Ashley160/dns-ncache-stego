...
int main(int argc, char *argv[]) {
    ...
    fprintf (stderr, "\nReceiving: %s  (%d bytes) threads=%d\n\n",
            nxdomain.c_str(), size, num_threads);
    ...
    for (int i = 0; i < size; ++i) {
        ...
        if ((int)workers.size() == num_threads || i == size - 1) {
            ...
            workers.clear();
            bytes_recv = i + 1;
            print_progress(bytes_recv, (size_t)size, 3, 3, "value", t_progress_start);
        }
    }
    print_progress((size_t)size, (size_t)size, 3, 3, "value", t_progress_start);
    fprintf(stderr, "\n\n");
